import { weibo, qq, qqZone, douban, shareUrl, shareTitle } from '@/util/share/env'
import * as mutils from '@/util/share/mUtils'
...
export function shareConfig (type, obj) { // 传参数obj
  let baseUrl = ''
  ...
  switch (type) {
    case 'weibo':
      ...
      window.open(baseUrl, '_blank') // 新开分享页地址拼接
      break
    case 'qq':
      const qqData = {
        'url': shareUrl,
    ...
    case 'qqZone':
      const qqZoneData = {
        'url': shareUrl,
        'title': shareTitle,
        'pics': obj.pic || (qqZone.pic).split(','), // 返回一个数组
